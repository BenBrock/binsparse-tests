...
from .config import discover_binsparse_fixtures
from .config import load_canonical_manifest
from .config import load_matrix_manifest
from .config import load_parser_manifest
from .contracts import CanonicalReferenceCase


LEGACY_FIXTURE_NAMES = {"matrix_fixture", "paired_matrix_fixture", "binsparse_fixture"}
...
def load_requested_legacy_fixtures(pytestconfig: pytest.Config) -> list:
    fixtures = []
...
def load_requested_canonical_fixtures(pytestconfig: pytest.Config) -> list:
...
def parametrize_requested_fixtures(metafunc: pytest.Metafunc) -> None:
    legacy_fixtures = None
    canonical_fixtures = None

    if "matrix_fixture" in metafunc.fixturenames:
        legacy_fixtures = legacy_fixtures or load_requested_legacy_fixtures(
            metafunc.config
        )
        metafunc.parametrize(
            ...
            legacy_fixtures,
            indirect=True,
            ids=[fixture.name for fixture in legacy_fixtures],
        )
    ...
    if "paired_matrix_fixture" in metafunc.fixturenames:
        ...
        paired_fixtures = [
            fixture for fixture in legacy_fixtures if fixture.matrix_market is not None
        ]
        metafunc.parametrize(
    ...
    if "canonical_fixture" in metafunc.fixturenames:
        canonical_fixtures = canonical_fixtures or load_requested_canonical_fixtures(
            metafunc.config
        )
        metafunc.parametrize(
            "canonical_fixture",
...
def filter_legacy_collection(config: pytest.Config, items: list[pytest.Item]) -> None:
    if not any(
        set(getattr(item, "fixturenames", ())) & LEGACY_FIXTURE_NAMES for item in items
    ):
        return

    fixtures = load_requested_legacy_fixtures(config)
...
def load_active_parser(pytestconfig: pytest.Config):
